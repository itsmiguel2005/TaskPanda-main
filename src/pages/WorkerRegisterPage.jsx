...
export default function WorkerRegisterPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    localStorage.setItem("workerStep1", JSON.stringify(step1));
    navigate("/worker-register/name");
  };
          ...
          <div className="w-full max-w-sm space-y-6">
            ...
            <div className="flex justify-center gap-1.5" aria-label="Registration progress">
              {[1, 2, 3, 4, 5].map((step) => (
                <span
                  key={step}
                  className={`h-1.5 w-5 rounded-full ${step === 1 ? "bg-green-600" : "bg-gray-200"}`}
                />
              ))}
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <input
                    ...
                    className={`block w-full rounded-lg border px-4 py-2.5 pr-11 text-sm text-gray-800 placeholder-gray-400/70 transition-colors focus:outline-none focus:ring-2 focus:ring-green-500/30 ${
              ...
              <div className="flex items-start gap-2">
                ...
                />
                <label htmlFor="agreeTermsWorker" className="text-sm text-gray-600">
                  I agree to the{" "}
                  <button type="button" onClick={() => setShowTerms(true)} className="font-medium text-green-600 underline">
                    Terms of Service
                  </button>{" "}
                  and{" "}
                  <button type="button" onClick={() => setShowTerms(true)} className="font-medium text-green-600 underline">
